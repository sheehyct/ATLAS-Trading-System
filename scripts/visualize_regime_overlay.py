...
def plot_regime_overlay(
    ...
    # Detect VIX spikes before trimming
    ...
    # Trim data to start from when regime detection begins (skip lookback period)
    regime_start_idx = regimes.first_valid_index()
    if regime_start_idx is not None:
        # Trim data to start from regime detection
        data_trimmed = data.loc[regime_start_idx:]
        regimes_trimmed = regimes.loc[regime_start_idx:]
        vix_spikes_trimmed = vix_spikes.loc[regime_start_idx:]
        print(f"   Chart trimmed: {data.index[0].strftime('%Y-%m-%d')} -> {regime_start_idx.strftime('%Y-%m-%d')} (removed {len(data) - len(data_trimmed)} days)")
    else:
        data_trimmed = data
        regimes_trimmed = regimes
        vix_spikes_trimmed = vix_spikes
    ...
    for regime_name, color in regime_colors.items():
        regime_mask = (regimes_trimmed == regime_name)

        if regime_mask.sum() == 0:
            continue  # Skip if regime never appears

        # Get regime periods
        regime_dates = regimes_trimmed[regime_mask].index
        ...
        for segment in segments:
            ...
            segment_data = data_trimmed.loc[segment]
    ...
    fig.add_trace(go.Scatter(
        x=data_trimmed.index,
        y=data_trimmed['Close'],
        mode='lines',
        name=f'{symbol} Close',
    ...
    # Calculate max drawdown and recovery time for each VIX spike
    vix_spike_details = []
    if vix_spikes_trimmed.sum() > 0:
        spike_dates = vix_spikes_trimmed[vix_spikes_trimmed].index
        spike_prices = data_trimmed.loc[spike_dates, 'Close']

        for spike_date in spike_dates:
            # Get price at spike
            spike_price = data_trimmed.loc[spike_date, 'Close']

            # Get future prices (next 10 days max)
            future_dates = data_trimmed.index[data_trimmed.index > spike_date]
            if len(future_dates) >= 1:
                # Look at next 5 days for max drawdown
                lookforward = min(5, len(future_dates))
                future_prices = data_trimmed.loc[future_dates[:lookforward], 'Close']

                # Calculate max drawdown from spike price
                max_drawdown = ((future_prices.min() - spike_price) / spike_price) * 100

                # Calculate recovery time (days to get back to spike price)
                recovery_mask = data_trimmed.loc[future_dates, 'Close'] >= spike_price
                if recovery_mask.any():
                    recovery_idx = recovery_mask.idxmax()  # First True
                    recovery_days = len(data_trimmed.loc[spike_date:recovery_idx]) - 1
                else:
                    recovery_days = None  # Not recovered

                vix_spike_details.append({
                    'date': spike_date,
                    'price': spike_price,
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=None,  # Remove "Date" label since dates are on axis
            showgrid=True,
            gridwidth=1,
            ...
            dtick="M3",  # Show every 3 months to prevent overlap
            tickformat="%b\n%Y",  # Month abbreviation + year on new line
            tickangle=0
        ),
        yaxis=dict(
        ...
        paper_bgcolor='white',
        height=750,
        width=1400,  # Narrower width to fit in screenshot
        margin=dict(l=80, r=250, t=120, b=100),  # Right margin for VIX sidebar
        ...
        annotations=[
            ...
            dict(
                text=(
                    f"Classified: {len(regimes_trimmed)} days  •  "
                    f"Coverage: {len(regimes)/len(data)*100:.1f}%  •  "
                    f"VIX Spikes: {vix_spikes_trimmed.sum() if vix_spikes_trimmed.sum() > 0 else 0}  •  "
            ...
            dict(
                text=(
                    "<b style='font-size:13px'>VIX SPIKE EVENTS</b><br>" +
                    "<span style='font-size:9px; color:#6c757d'>Max DD | Recovery</span><br><br>" +
                    ("<br>".join([
                        f"<b style='font-size:9px'>{detail['date'].strftime('%b %d, %Y')}</b><br>"
                        f"<span style='font-size:10px; color:#DC3545'>{detail['max_drawdown']:.1f}%</span> | "
                        f"<span style='font-size:10px; color:{'#28A745' if detail['recovery_days'] is not None else '#95a5a6'}'>"
                        f"{detail['recovery_days']}d" if detail['recovery_days'] is not None else "N/A"
                        f"</span>"
                        for detail in vix_spike_details
                    ]) if vix_spike_details else "<span style='color:#95a5a6'>No spikes detected</span>")
                ),
                xref="paper", yref="paper",
                x=1.01, y=0.5,  # Positioned at right edge with minimal gap
                showarrow=False,
    ...
    # Save to HTML (organized in visualization directory)
    if save_path is None:
        save_path = f'visualization/regime_overlays/{symbol}_regime_overlay.html'
...
def create_comparison_chart(symbols=['SPY', 'QQQ'], start_date='2020-01-01', end_date='2025-11-14'):
    ...
    # Create subplots (no subplot_titles to avoid overlap with legend)
    fig = make_subplots(
        rows=len(symbols),
        cols=1,
    ...
    for idx, symbol in enumerate(symbols, start=1):
        ...
        # Trim data to start from when regime detection begins
        regime_start_idx = regimes.first_valid_index()
        if regime_start_idx is not None:
            data_trimmed = data.loc[regime_start_idx:]
            regimes_trimmed = regimes.loc[regime_start_idx:]
    ...
    # Add subplot titles as annotations (positioned below main title)
    subplot_annotations = []
    for idx, symbol in enumerate(symbols, start=1):
        # Calculate y position for each subplot title
        # For 2 subplots: top subplot gets y=0.92, bottom gets y=0.42
        y_position = 1.0 - (idx - 1) * (1.0 / len(symbols)) - 0.05
        subplot_annotations.append(
            dict(
                text=f'<b>{symbol} with ATLAS Regime Detection</b>',
                xref="paper", yref="paper",
                x=0.5, y=y_position,
                showarrow=False,
    ...
    fig.update_layout(annotations=subplot_annotations)

    # Save (organized in visualization directory)
    save_path = 'visualization/regime_overlays/Combined_regime_comparison.html'
    fig.write_html(save_path)
